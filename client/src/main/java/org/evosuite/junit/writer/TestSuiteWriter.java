...
import org.evosuite.spring.SpringClassTestContext;
...
public class TestSuiteWriter implements Opcodes {
    ...
    private final boolean usesSpring;

    public TestSuiteWriter() {
        this(false);
    }

    public TestSuiteWriter(boolean usesSpring) {
    ...
    protected String getImports(List<ExecutionResult> results) {
        ...
        if (usesSpring){
            imports.addAll(SpringSupport.getImports());
        }
    ...
    /**
     * JUnit file header until test suite definition with fields.
     *
    ...
    protected String getHeader(String testName, String scaffoldingName, List<ExecutionResult> results) {
        ...
        addEvoSuiteHeader(builder);
        addPackage(builder);
        addImports(builder, results);
        addRunner(builder);
        addTestSuiteDefinition(builder, testName, scaffoldingName);
        addTestSuiteFields(builder, testName);

        return builder.toString();
    }

    public static void addEvoSuiteHeader(StringBuilder builder){
        builder.append("/*").append(NEWLINE);
        builder.append(" * This file was automatically generated by EvoSuite").append(NEWLINE);
        builder.append(" * " + new Date()).append(NEWLINE);
        builder.append(" */").append(NEWLINE);
        builder.append(NEWLINE);
    }

    private static void addPackage(StringBuilder builder) {
    ...
    private void addRunner(StringBuilder builder) {
        if (usesSpring){
    ...
    private void addTestSuiteDefinition(StringBuilder builder, String testName, String scaffoldingName) {
        builder.append(adapter.getClassDefinition(testName));
        if (!usesSpring && Properties.TEST_SCAFFOLDING && !Properties.NO_RUNTIME_DEPENDENCY) {
    ...
    private void addTestSuiteFields(StringBuilder builder, String testName){
        if (Properties.TEST_FORMAT == Properties.OutputFormat.JUNIT5) {
            builder.append("@RegisterExtension").append(NEWLINE);
            builder.append(METHOD_SPACE).append("static EvoRunnerJUnit5 runner = new EvoRunnerJUnit5(").append(testName).append(".class);").append(NEWLINE);
        }
        if (usesSpring){
            SpringSupport.addFields(builder);
        }
    ...
    protected String testToString(int number, int id, ExecutionResult result) {
        ...
        for (String line : adapter.getMethodDefinition(methodName).split(NEWLINE)) {
            builder.append(METHOD_SPACE).append(line).append(NEWLINE);
        }
        ...
        // No code after an exception should be printed as it would break compatibility
